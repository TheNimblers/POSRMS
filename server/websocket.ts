...
class WebSocketManager {
  ...
  private handleConnection(ws: WebSocket, request: IncomingMessage) {
    ...
    console.log(`Client connected: ${userId} (${role}) - Restaurant: ${restaurantId}`);
  ...
  private handleOrderPlaced(event: WebSocketEvent, restaurantId?: string) {
    ...
    if (data.items) {
      ...
      if (foodItems.length > 0) {
        ...
        }, ['kitchen', 'manager', 'admin'], restaurantId);
      }
      ...
      if (drinkItems.length > 0) {
        ...
        }, ['bar', 'manager', 'admin'], restaurantId);
      }
    }
  }
  ...
  private handleCallWaiter(event: WebSocketEvent, restaurantId?: string) {
    ...
    }, ['waiter', 'manager', 'admin'], restaurantId);
  }
  ...
  private handleOrderStatusUpdated(event: WebSocketEvent, restaurantId?: string) {
  ...
  private broadcastToRoles(event: WebSocketEvent, roles: string[], restaurantId?: string) {
  ...
  private broadcast(event: WebSocketEvent, restaurantId?: string, excludeClientId?: string) {
  ...
  public notifyTableActivated(tableId: number, tableNumber: string, restaurantId: string) {
  ...
  public getStats() {
    ...
    this.clients.forEach((client) => {
      ...
      if (client.restaurantId) {
        stats.byRestaurant[client.restaurantId] = (stats.byRestaurant[client.restaurantId] || 0) + 1;
      }
    });
