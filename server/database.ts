...
class DatabaseService {
  ...
  private async seedDatabase() {
    ...
    const restaurantCount = this.db.prepare('SELECT COUNT(*) as count FROM restaurants').get() as { count: number };
    ...
    try {
      ...
      `).run(
        restaurant.id, restaurant.name, restaurant.slug, restaurant.subscription_status,
        restaurant.subscription_plan, restaurant.admins, restaurant.contact_email,
        restaurant.address, restaurant.phone, restaurant.timezone, restaurant.currency,
        restaurant.tax_rate, restaurant.service_charge, restaurant.settings
      );
      ...
      const staff = [
        {
          ...
          name: 'John Doe',
          email: 'john@demo.com',
          permissions: JSON.stringify(['view_tables', 'manage_orders', 'update_order_status']),
          shift: 'evening'
        },
        {
          id: 'waiter2',
        ...
        },
        {
          ...
          name: 'Mike Johnson',
          email: 'mike@demo.com',
          permissions: JSON.stringify(['view_food_orders', 'update_food_status']),
          shift: 'evening'
        },
        {
          id: 'bar1',
          ...
          permissions: JSON.stringify(['view_drink_orders', 'update_drink_status']),
          shift: 'day'
        },
        {
          id: 'manager1',
          ...
          permissions: JSON.stringify(['view_tables', 'manage_orders', 'manage_menu', 'manage_staff', 'view_analytics']),
          shift: 'day'
        },
        {
          id: 'admin1',
        ...
        },
        {
          ...
          permissions: JSON.stringify(['manage_restaurants', 'manage_subscriptions', 'view_global_analytics'])
        }
      ];
      ...
      for (const member of staff) {
        ...
        `).run(
          member.id, member.username, passwordHash, member.role,
          member.role === 'team' ? null : restaurantId,
          member.name, member.email, member.permissions, member.shift || null
        );
      }
      ...
      for (const item of menuItems) {
        ...
        `).run(
          item.id, restaurantId, item.name, item.description, item.category,
          item.price_eur, item.price_usd, item.special || false, item.preparation_time
        );
      }
