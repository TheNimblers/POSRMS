...
export const handleCreateOrder: RequestHandler = async (req, res) => {
  try {
    ...
    for (const item of data.items) {
      ...
      if (
        ["starter", "main", "dessert", "special"].includes(menuItem.category)
      ) {
        hasFood = true;
      } else {
        hasDrinks = true;
...
export const handleUpdateOrderStatus: RequestHandler = async (req, res) => {
  try {
    ...
    // Get updated order
    const updatedOrder = db.queryOne("SELECT * FROM orders WHERE id = ?", [
      orderId,
    ]) as Order;
...
export const handleMarkSessionPaid: RequestHandler = async (req, res) => {
  try {
    ...
    if (!session) {
      return res
        .status(404)
        .json({ success: false, error: "Session not found" } as ApiResponse);
    }
