...
export const handleLogin: RequestHandler = async (req, res) => {
  try {
    ...
    // Verify password
    const isPasswordValid = await db.comparePassword(password, user.password_hash);
    if (!isPasswordValid) {
      return res.status(401).json({
        success: false,
        error: 'Invalid credentials'
...
export const handleRegister: RequestHandler = async (req, res) => {
  try {
    ...
    `, [
      userId, data.username, passwordHash, data.role, data.restaurantId,
      data.name, data.email, JSON.stringify(permissions), data.shift || null, 'active'
    ]);
...
export const handleUpdatePassword: RequestHandler = async (req, res) => {
  try {
    ...
    const isCurrentPasswordValid = await db.comparePassword(currentPassword, user.password_hash);
...
function getPermissionsByRole(role: string): string[] {
  switch (role) {
    ...
    case 'manager':
      return ['view_tables', 'manage_orders', 'manage_menu', 'manage_staff', 'view_analytics'];
    case 'admin':
      return ['full_access'];
    case 'team':
      return ['manage_restaurants', 'manage_subscriptions', 'view_global_analytics'];
    default:
      return [];
  }
}
