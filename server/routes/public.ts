...
export const handleStartPublicSession: RequestHandler = async (req, res) => {
  try {
    ...
    if (!table) {
      return res.status(404).json({ success: false, error: 'Invalid table token' } as ApiResponse);
    }
    ...
    if (!session) {
      ...
      session = db.queryOne('SELECT * FROM sessions WHERE id = ?', [sessionId]) as Session;
    }

    // Mark table active
    db.execute('UPDATE tables SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', ['active', table.id]);
    webSocketManager.notifyTableActivated(table.id, table.number, table.restaurant_id);

    res.status(201).json({ success: true, data: session, message: 'Session started' } as ApiResponse);
  } catch (error) {
    console.error('Start public session error:', error);
    if (error instanceof z.ZodError) {
      return res.status(400).json({ success: false, error: 'Validation error', message: error.errors[0].message } as ApiResponse);
    }
...
export const handleCreatePublicOrder: RequestHandler = async (req, res) => {
  try {
    const { token, items, currency = 'USD', notes } = createPublicOrderSchema.parse(req.body);
    ...
    for (const item of items) {
      ...
      if (!menuItem) {
        return res.status(404).json({ success: false, error: `Menu item not found: ${item.menuItemId}` } as ApiResponse);
      }

      const price = currency === 'EUR' ? menuItem.price_eur : menuItem.price_usd;
      ...
      if (['starter', 'main', 'dessert', 'special'].includes(menuItem.category)) hasFood = true;
      else hasDrinks = true;
    }
    ...
    `, [
      orderId, orderNumber, session.id, table.restaurant_id, table.id,
      JSON.stringify(orderItems), orderType, totalAmount, notes || null
    ]);

    // Update session total
    db.execute('UPDATE sessions SET total_amount = total_amount + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [totalAmount, session.id]);
    ...
    const newOrder = db.queryOne('SELECT * FROM orders WHERE id = ?', [orderId]);

    res.status(201).json({ success: true, data: { ...newOrder, items: orderItems }, message: 'Order placed' } as ApiResponse);
  } catch (error) {
...
export const handlePublicCallWaiter: RequestHandler = async (req, res) => {
  try {
    ...
    webSocketManager['broadcastToRoles' as any]({
      type: 'call_waiter',
      data: { tableId: table.id, tableNumber: table.number, message: 'Customer requested assistance' },
      timestamp: new Date().toISOString()
    }, ['waiter', 'manager', 'admin'], table.restaurant_id);
