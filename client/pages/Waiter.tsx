...
type TableStatus = 'available' | 'active' | 'waiting_payment' | 'needs_attention';
...
export default function Waiter() {
  ...
  const myTables = tables.filter(table => table.assignedWaiter === user?.username);
  const unassignedTables = tables.filter(table => table.status === 'active' && !table.assignedWaiter);
  const allActiveTables = tables.filter(table => table.status !== 'available');

  // Get notifications count
  const notificationCount = myTables.reduce((count, table) => count + table.notifications.length, 0);

  const claimTable = (tableId: number) => {
    setTables(tables.map(table => 
      table.id === tableId 
        ? { ...table, assignedWaiter: user?.username || '', notifications: table.notifications.filter(n => n !== 'new_table') }
        : table
    ));
  };
  ...
  const handleNotification = (tableId: number, notification: string) => {
    setTables(tables.map(table => 
      table.id === tableId
        ? { 
            ...table, 
            notifications: table.notifications.filter(n => n !== notification),
        ...
        <div className="space-y-2">
          ...
          {table.assignedWaiter && (
            <div className="flex items-center text-sm text-gray-600">
              ...
              {table.assignedWaiter === user?.username ? 'You' : table.assignedWaiter}
            </div>
          )}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {table.notifications.includes('payment_requested') && (
                <Button 
                  size="sm" 
                  variant="outline"
                  onClick={() => handleNotification(table.id, 'payment_requested')}
                >
                  Process Payment
                </Button>
              )}

              {table.currentOrder && table.assignedWaiter === user?.username && (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <Badge className="ml-4 bg-blue-100 text-blue-800">Waiter Dashboard</Badge>
            </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{myTables.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-6">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{unassignedTables.length}</p>
                </div>
              </div>
            </CardContent>
          </Card>
            ...
            {myTables.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">No tables assigned</h3>
                  <p className="text-gray-600">Check the unassigned tab to claim new tables.</p>
                </CardContent>
              </Card>
            ) : (
            ...
            {unassignedTables.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-gray-900 mb-2">All tables assigned</h3>
                  <p className="text-gray-600">Great job! All active tables have been claimed.</p>
                </CardContent>
              </Card>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
